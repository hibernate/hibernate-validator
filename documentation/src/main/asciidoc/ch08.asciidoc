...
link:http://docs.oracle.com/javase/8/docs/technotes/guides/jar/jar.html#Service%20Provider[Java
...
You can obtain the `ClockProvider` in your validators via the `ConstraintValidatorContext`.

For instance, this might be useful if you want to replace the default message of the `@Future` constraint with a more
explicit one.
====
