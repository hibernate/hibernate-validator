...
`@Negative(strict=)`:: Checks if the element is negative. If `strict=true` zero values are considered invalid, otherwise they are valid.
	Supported data types::: `BigDecimal`, `BigInteger`, `byte`, `short`, `int`, `long` and the respective wrappers of the primitive types; additionally supported by HV: any sub-type of `CharSequence` (the numeric value represented by the character sequence is evaluated), any sub-type of `Number` and `javax.money.MonetaryAmount`
	Hibernate metadata impact::: None
...
`@Positive(strict=)`:: Checks if the element is positive. If `strict=true` zero values are considered invalid, otherwise they are valid.
	Supported data types::: `BigDecimal`, `BigInteger`, `byte`, `short`, `int`, `long` and the respective wrappers of the primitive types; additionally supported by HV: any sub-type of `CharSequence` (the numeric value represented by the character sequence is evaluated), any sub-type of `Number` and `javax.money.MonetaryAmount`
	Hibernate metadata impact::: None
...
`@Valid`:: Performs validation recursively on the associated object. If the object is a collection or an array, the elements are validated recursively. If the object is a map, the value elements are validated recursively.
...
`@Email`:: Checks whether the specified character sequence is a valid email address. The optional parameters `regexp` and `flags` allow to specify an additional regular expression (including regular expression flags) which the email must match. Is *deprecated* in favor of corresponding Bean Validation annotation (see <<validator-defineconstraints-spec>>).
	Supported data types::: `CharSequence`
	Hibernate metadata impact::: None
...
`@NotBlank`:: Checks that the annotated character sequence is not null and the trimmed length is greater than 0. The difference to `@NotEmpty` is that this constraint can only be applied on strings and that trailing white-spaces are ignored.  Is *deprecated* in favor of corresponding Bean Validation annotation (see <<validator-defineconstraints-spec>>).
	Supported data types::: `CharSequence`
	Hibernate metadata impact::: None

`@NotEmpty`:: Checks whether the annotated element is not null nor empty.  Is *deprecated* in favor of corresponding Bean Validation annotation (see <<validator-defineconstraints-spec>>).
	Supported data types::: `CharSequence`, `Collection`, `Map` and arrays
	Hibernate metadata impact::: None
