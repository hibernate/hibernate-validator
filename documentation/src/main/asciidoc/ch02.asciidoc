...
|`@Pattern(regexp=,flags=)`|`CharSequence`|Checks if the annotated string matches the regular
...
this is a type level constraint, one can use `reportOn` attribute
to be able to report a constraint violation on a specified property
