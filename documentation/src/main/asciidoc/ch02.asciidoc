...
|`@TituloEleitoral`|`CharSequence`|Checks that the annotated character sequence represents
                a Brazilian voter ID card number (http://ghiorzi.org/cgcancpf.htm[Título Eleitoral])|Brazil|None
...
|`@REGON`|`CharSequence`|Checks that the annotated character sequence represents a Polish taxpayer identification
				number (https://pl.wikipedia.org/wiki/REGON[REGON]). Can be applied to both 9 and 14 digit versions of REGON|Poland|None
