...
how script evaluators (`ScriptEvaluator`) are built, and which scripting engines are used to build
them. This can be done by setting a custom implementation of `ScriptEvaluatorFactory`. In
particular this is important for modular environments (e.g. OSGi), where user might face issues
...
To configure it programmatically, an instance of `ScriptEvaluatorFactory` should be passed when building the `ValidatorFactory`.
...
This section shows a couple of custom `ScriptEvaluatorFactory` implementations which can be used
in modular environments as well as one using https://docs.spring.io/spring/docs/current/spring-framework-reference/core.html#expressions[Spring Expression Language]
for writing constraint scripts.

Problems with modular environments and https://www.jcp.org/en/jsr/detail?id=223[JSR-223] is caused by the classloading. `ScriptEngineManager`
used to find a scripting engine might be using a different classloader from the one where the scripting engine is actually available.
To solve this problem a `MultiClassloaderScriptEvaluatorFactory` can be introduced:

====
[source, JAVA, indent=0]
----
include::{sourcedir}/org/hibernate/validator/referenceguide/chapter09/MultiClassloaderScriptEvaluatorFactory.java[tags=include]
----
====
...
This way, it is possible to pass multiple `ClassLoader` instances, which will be used to search for
the needed `ScriptEngine`.

An alternative approach for OSGi environment can be usage of next `OSGiScriptEvaluatorFactory`:

====
[source, JAVA, indent=0]
----
include::{sourcedir}/org/hibernate/validator/referenceguide/chapter09/OSGiScriptEvaluatorFactory.java[tags=include]
----
====

It is designed specifically for OSGi environments and allows user to pass `BundleContext` as a
parameter which will be used to search for `ScriptEngineFactory`.

As it was already said, it is also possible to use other than https://www.jcp.org/en/jsr/detail?id=223[JSR-223]
based script evaluation (e.g. https://docs.spring.io/spring/docs/current/spring-framework-reference/core.html#expressions[Spring Expression Language]).
`SpringELEvaluatorFactory` can be written like:

====
[source, JAVA, indent=0]
----
include::{sourcedir}/org/hibernate/validator/referenceguide/chapter09/SpringELEvaluatorFactory.java[tags=include]
----
====

This factory would allow usage of Spring expression language in `ScriptAssert` and `ParameterScriptAssert`
constraints:
