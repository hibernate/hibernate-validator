...
| `strengthEstimator(int minScore, PasswordStrengthEstimator)` | Requires a minimum strength score using a `PasswordStrengthEstimator` obtained from the initialization context (see <<section-password-validation-services>>)
| `compromisedChecker(CompromisedPasswordChecker)` | Rejects passwords flagged as compromised using a `CompromisedPasswordChecker` obtained from the initialization context (see <<section-password-validation-services>>)
|===
