...
| `ScriptEvaluatorFactory` | Controls how script engines are initialized for `@ScriptAssert` and `@ParameterScriptAssert`
| `GetterPropertySelectionStrategy` | Determines which methods are considered property getters
| `PropertyNodeNameProvider` | Resolves property names in constraint violation paths
| `LocaleResolver` | Resolves the locale for message interpolation
| `BeanMetaDataClassNormalizer` | Normalizes bean classes (e.g. for proxy unwrapping)
...
| `PasswordPolicyDefinitionResolver` | Resolves `PasswordPolicyDefinition` instances for `@PasswordPolicy`
| `MessageInterpolator` | Interpolates constraint violation messages
|===

Most of these components also support configuration via the dedicated programmatic setters
on the Hibernate Validator configuration object (e.g. `scriptEvaluatorFactory()`, `localeResolver()`)
or via properties. The explicit setter takes priority over bean-registered implementations.
