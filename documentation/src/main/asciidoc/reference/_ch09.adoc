...
[[section-bean-system]]
==== Bean resolution and pluggable components

Hibernate Validator uses a bean resolution system to manage its pluggable components.

===== Registering beans programmatically

Use `addBeanConfigurer()` on the Hibernate Validator configuration to register custom implementations:
...
Validator validator = Validation.byProvider( HibernateValidator.class )
		...
		.addBeanConfigurer( context -> {
			context.define( PasswordStrengthEstimator.class,
					BeanReference.ofInstance( new MyPasswordStrengthEstimator() ) );
			context.define( CompromisedPasswordChecker.class,
					BeanReference.ofInstance( new MyCompromisedPasswordChecker() ) );
		} )
...
Each `define()` call associates an implementation with a type (and optionally a name).

===== CDI integration

In CDI environments, when using the `hibernate-validator-cdi` extension,
Hibernate Validator automatically discovers a `BeanProvider`
backed by the CDI `BeanManager`. Any bean available in the CDI container can be
resolved -- no explicit registration is needed.

For other DI frameworks, set a custom `BeanProvider` via the configuration:
...
Validation.byProvider( HibernateValidator.class )
		.configure()
		.beanProvider( myCustomBeanProvider )
		.buildValidatorFactory();
----
====
...
`BeanConfigurer` implementations discovered via `java.util.ServiceLoader` are applied
automatically at bootstrap. Create a file named
`META-INF/services/org.hibernate.validator.spi.bean.BeanConfigurer` containing
the fully-qualified class names of your configurers.

===== Pluggable components

The following components can be configured through the bean system.

====== Components with builtin defaults

These components have a builtin default implementation. To override the default, you must
either use the programmatic setter on the configuration object or set the corresponding
configuration property. The property value is resolved as a bean reference through the
bean system -- so you can register a custom implementation, e.g. via `addBeanConfigurer()`, and
then select it using the property.

The resolution order is: explicit programmatic setter > configuration property > builtin default.

[cols="2,3,2", options="header"]
|===
| Component | Description | Property

| `ScriptEvaluatorFactory` | Controls how script engines are initialized for `@ScriptAssert` and `@ParameterScriptAssert` | `hibernate.validator.script_evaluator_factory`
| `GetterPropertySelectionStrategy` | Determines which methods are considered property getters | `hibernate.validator.getter_property_selection_strategy`
| `PropertyNodeNameProvider` | Resolves property names in constraint violation paths | `hibernate.validator.property_node_name_provider`
| `LocaleResolver` | Resolves the locale for message interpolation | `hibernate.validator.locale_resolver`
| `BeanMetaDataClassNormalizer` | Normalizes bean classes (e.g. for proxy unwrapping) | `hibernate.validator.bean_meta_data_class_normalizer`
| `PasswordPolicyDefinitionResolver` | Resolves `PasswordPolicyDefinition` instances for `@PasswordPolicy` | `hibernate.validator.password_policy_definition_resolver`
| `MessageInterpolator` | Interpolates constraint violation messages | `hibernate.validator.message_interpolator`
|===

====== Components without defaults

These components have no builtin default. Registering a single implementation is
sufficient -- the validator will auto-discover it. When multiple implementations
are registered, you would need to resolve the ambiguity. For example, in case of
password-related constraints, use the bean reference in the constraint annotation
attribute to select which one to use (e.g. `@PasswordStrength(estimator = "myEstimator")`).

[cols="2,3", options="header"]
|===
| Component | Description

| `PasswordStrengthEstimator` | Estimates password strength for `@PasswordStrength`
| `CompromisedPasswordChecker` | Checks for compromised passwords for `@NotCompromised`
|===
...
==== Configuring password validation beans

[WARNING]
====
The security of password validation depends entirely on your implementations of
`PasswordStrengthEstimator`, `CompromisedPasswordChecker`, and any custom `PasswordPolicyRule`.
Implementations must handle password data securely. Check with the most current rules and
standards for handling passwords. Following are a few examples of what to check for
(applicable at the time of writing this documentation): never send plaintext passwords to
external services, never log passwords, and zero `char[]` arrays after use.

The example implementations in this section are provided for illustration only and are *not*
production-ready -- they lack error handling, timeouts, retries, and circuit breaking that
a production deployment would require.
====

The `@PasswordStrength` and `@NotCompromised` constraints delegate to user-provided implementations
that must be registered as beans at bootstrap time. The two interfaces are:

* `PasswordStrengthEstimator` -- estimates password strength (the score scale is defined by the implementation)
...
Register beans via the `addBeanConfigurer()` method on the Hibernate Validator configuration:
...
[CAUTION]
====
This example is for illustration only. A production implementation should include proper error handling,
timeouts, retries with backoff, and circuit breaking. Consider caching results and respecting the
API's rate limits. Always verify that your implementation never sends the full password or
its complete hash to any external service.
====
...
| `strengthEstimator(int minScore, PasswordStrengthEstimator)` | Requires a minimum strength score using a `PasswordStrengthEstimator` (see <<section-password-validation-services>>)
| `compromisedChecker(CompromisedPasswordChecker)` | Rejects passwords flagged as compromised using a `CompromisedPasswordChecker` (see <<section-password-validation-services>>)
|===
...
These services are application concerns -- they should be injected by your DI framework.
...
For dependency injection or custom initialization, register a custom `PasswordPolicyDefinitionResolver`:
...
[[section-password-policy-class-level]]
===== Class-level validation with `AbstractPasswordPolicyValidator`

When a password policy rule needs access to other bean properties -- for example, to reject passwords
that contain the username -- `@PasswordPolicy` can be placed on the class instead of the password field.

Hibernate Validator does not ship a class-level validator; you provide your own by extending
`AbstractPasswordPolicyValidator<T>`. This base class resolves the policy definition, builds
the rules, and runs the validation. You only need to implement two methods:

* `getPassword(T)` -- extracts the password from the bean
* `bindProperties(T, BiConsumer<String, Object>)` -- binds additional properties that rules can read
via `PasswordContext.get(name, type)`

For example, first, define a rule that reads a property from the context:

[[example-password-policy-no-username-rule]]
.A rule that rejects passwords containing the username
====
[source, java, indent=0]
----
include::{sourcedir}/org/hibernate/validator/referenceguide/chapter09/PasswordPolicyTest.java[tags=noUsernameRule]
----
====

Define a policy that uses this rule alongside standard rules:

[[example-password-policy-class-level-policy]]
.A policy definition using cross-field rules
====
[source, java, indent=0]
----
include::{sourcedir}/org/hibernate/validator/referenceguide/chapter09/PasswordPolicyTest.java[tags=classLevelPolicy]
----
====

Annotate the bean with `@PasswordPolicy` at the class level:

[[example-password-policy-class-level-bean]]
.A bean with class-level `@PasswordPolicy`
====
[source, java, indent=0]
----
include::{sourcedir}/org/hibernate/validator/referenceguide/chapter09/PasswordPolicyTest.java[tags=classLevelBean]
----
====

Implement the validator by extending `AbstractPasswordPolicyValidator`:

[[example-password-policy-class-level-validator]]
.Class-level validator implementation
====
[source, java, indent=0]
----
include::{sourcedir}/org/hibernate/validator/referenceguide/chapter09/PasswordPolicyTest.java[tags=classLevelValidator]
----
====

Since this is a custom validator for an existing constraint, it must be registered with Hibernate
Validator. This can be done via XML, the `ServiceLoader` mechanism, or the programmatic API.
See <<section-constraint-definition-contribution>> for details.
