...
The following components are required at factory construction time and have a simpler resolution.
When configured through a property, the property value is treated as a fully-qualified class name
and the component is instantiated via its no-arg constructor.
...
More complex rules that depend on external resources -- dictionary lookups, password history checks,
breach databases -- should delegate to an external service interface. The application provides the
implementation; the rule only needs the contract:
...
The rule receives the service via its constructor and delegates to it:
...
`PasswordPolicyDefinition` classes are resolved through the bean system
(see <<section-bean-system>>). By default, they are instantiated via their no-arg constructor.
In other environments, you can make the definition a managed bean --
its dependencies will be injected by the container. You can also register it
via `addBeanConfigurer()` to wire dependencies programmatically:
