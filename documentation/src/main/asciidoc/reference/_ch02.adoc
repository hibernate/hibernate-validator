...
`@Trimmed`:: Checks that the annotated character sequence has no leading or trailing whitespace. A character is considered whitespace according to `Character.isWhitespace(int)`, matching `String.strip()` semantics. `null` and empty sequences are considered valid.
	Supported data types::: `CharSequence`
	Hibernate metadata impact::: None
