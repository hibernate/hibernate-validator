...
`@URI(type=, schemes=, authority=, host=, allowOpaque=, regexp=, flags=)`:: Checks if the annotated character sequence is a valid URI according to RFC 3986 using `java.net.URI`. Accepts all URI forms including absolute URIs (e.g., `http://example.com`), relative references (e.g., `../path`), and opaque URIs (e.g., `mailto:user@host`, `urn:isbn:123`). `null` and empty string are considered valid. The optional parameter `type` controls whether absolute, relative, or any URI is accepted. `schemes` restricts allowed URI schemes (case-insensitive). `authority` controls the presence of the authority component (`//host:port`). `host` restricts the host format (domain, IPv4, IPv6). `allowOpaque` controls acceptance of opaque URIs. The optional parameters `regexp` and `flags` allow additional regex validation.
	Supported data types::: `CharSequence`
	Hibernate metadata impact::: None
