...
`@BIC`:: Checks that the annotated character sequence is a valid https://en.wikipedia.org/wiki/ISO_9362[BIC] (Bank Identifier Code / SWIFT code) per ISO 9362. The structural format is validated: proper length (8 or 11 characters), valid character sets per position, and valid ISO 3166-1 alpha-2 country code. Optional filtering by country codes (`countryCodes`) and bank codes (`bankCodes`) is supported.
	Supported data types::: `CharSequence`
	Hibernate metadata impact::: None
