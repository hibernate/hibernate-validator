...
`@Contains(value=, minRequired=, ignoreCase=)`:: Validates that the annotated character sequence contains the specified substrings. By default, all substrings must be present. Use `minRequired` to specify a minimum number of substrings that must match (e.g., `minRequired=1` for an OR-style check). When `ignoreCase` is set to `true`, the comparison is case-insensitive.
	Supported data types::: `CharSequence`
	Hibernate metadata impact::: None
...
`@DateTimeFormat`:: Checks that the annotated character sequence is valid for the date and/or time format. The `pattern` parameter specifies the date/time format pattern (e.g., "yyyy-MM-dd", "dd/MM/yyyy HH:mm:ss") following `java.time.format.DateTimeFormatter` conventions. The `locale` parameter accepts one or more locale strings (e.g., `"en-US"`, `{"en-US", "fr-FR"}`), validation succeeds if the value parses with any of them; defaults to `"ROOT"` which works for numeric patterns but not text-based ones like month names. The `lenient` parameter determines date validation behaviour: when `false` (default), invalid dates like February 31st are rejected; when `true`, invalid dates are accepted and rolled over to subsequent months (e.g., February 30th becomes March 2nd). `null` values are always considered valid.
	Supported data types::: `CharSequence`
	Hibernate metadata impact::: None
...
`@Hexadecimal(letterCase=, strictness=, allowEmpty=, prefix=)`:: Validates that the annotated character sequence contains only hexadecimal characters (`0-9`, `a-f`, `A-F`). `null` is always valid. `letterCase` accepts `LOWER_CASE` (only `0-9a-f`), `UPPER_CASE` (only `0-9A-F`) or `INSENSITIVE` (default, any mix of cases). `strictness` accepts `STRICT` (default, ASCII characters only) or `LENIENT` (additionally accepts the fullwidth forms `０-９`, `ａ-ｆ`, `Ａ-Ｆ`). An empty character sequence is invalid by default; set `allowEmpty` to allow it. `prefix` is a leading marker that must be present when non-empty; the well-known prefixes `0x` and `#` are matched exactly, while any other value is treated as a regular expression anchored to the start of the value. The `letterCase` and `strictness` attributes only apply to the characters following the prefix.
	Supported data types::: `CharSequence`
	Hibernate metadata impact::: None
...
`@NullOrNotEmpty`:: Validates that the annotated element is either `null` or not empty. This is useful for nullable fields and for use with `Optional`, where an empty `Optional` (extracted as `null`) should be considered valid, but a present value must not be empty.
	Supported data types::: `CharSequence`, `Collection`, `Map` and arrays
	Hibernate metadata impact::: None
...
`@CUIL`:: Checks that the annotated character sequence represents an Argentine Unique Labor Identification Code (CUIL)
	Supported data types::: `CharSequence`
	Hibernate metadata impact::: None
	Country::: Argentina

`@CUIT`:: Checks that the annotated character sequence represents an Argentine Unique Tax Identification Code (CUIT)
	Supported data types::: `CharSequence`
	Hibernate metadata impact::: None
	Country::: Argentina
...
`@RUC`:: Checks that the annotated character sequence represents a Paraguayan Unique Taxpayer Registry number (https://www.dnit.gov.py/documents/20123/224893/D%C3%ADgito%2BVerificador.pdf[DNIT RUC]). The base number may contain ASCII letters.
	Supported data types::: `CharSequence`
	Hibernate metadata impact::: None
	Country::: Paraguay
