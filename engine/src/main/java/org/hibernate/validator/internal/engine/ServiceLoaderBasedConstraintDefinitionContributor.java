...
public class ServiceLoaderBasedConstraintDefinitionContributor implements ConstraintDefinitionContributor {
	...
	@Override
	public void collectConstraintDefinitionContributions(ConstraintDefinitionBuilder constraintDefinitionContributionBuilder) {
	...
	private Class<?> determineAnnotationType(Class<?> constraintValidatorClass) {
		...
		//  constraintValidator cannot be null
		return constraintValidator.getTypeBindings().getTypeParameters().get( 0 ).getErasedType();
	}
