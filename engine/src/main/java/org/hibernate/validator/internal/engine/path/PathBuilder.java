 ...
 */
public final class PathBuilder implements Path {
	 ...
	 */
	public static PathBuilder createPathFromString(String propertyPath) {
	...
	public static PathBuilder createPathForExecutable(ExecutableMetaData executable) {
		Contracts.assertNotNull( executable, "A method is required to create a method return value path." );

		PathBuilder path = createRootPath();
	...
	public static PathBuilder createRootPath() {
		PathBuilder path = new PathBuilder();
		path.addBeanNode();
		return path;
	}

	public PathBuilder getPathWithoutLeafNode() {
		return new PathBuilder( new ArrayList<>( nodeList.subList( 0, nodeList.size() - 1 ) ) );
	}
...
//	}
	...
	public PathImpl build() {
		return new PathImpl( new ArrayList<>( nodeList ) );
	}

	private PathBuilder() {
		nodeList = new ArrayList<>( 5 );
	}

	private PathBuilder(List<Node> nodeList) {
		this.nodeList = nodeList;
		currentLeafNode = (NodeImpl) nodeList.get( nodeList.size() - 1 );
	}

	private static PathBuilder parseProperty(String propertyName) {
		PathBuilder path = createRootPath();
