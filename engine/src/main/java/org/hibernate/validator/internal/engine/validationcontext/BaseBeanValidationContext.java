...
public interface BaseBeanValidationContext<T> extends ValidationContext<T> {
	...
	/**
	 * @return {@code true} if current validation context can and should process passed meta constraint.
	 * 		Is used in {@link ValidatorImpl} to check if validation is required in case of calls to
	 * 		{@link Validator#validateValue(Class, String, Object, Class[])} or {@link Validator#validateProperty(Object, String, Class[])}.
	 * 		In these cases as we iterate through all meta constraints of entire bean we can encounter those that are not
	 * 		defined for the validated property.
	 */
