...
public class DefaultTraversableResolver implements TraversableResolver {
	...
	private void detectJPA() {
		...
		try {
			persistenceClass = ReflectionHelper.loadClass( ACCESS_CONTROL_CONTEXT, PERSISTENCE_CLASS_NAME, this.getClass() );
		}
		...
		Method persistenceUtilGetter = ReflectionHelper.getMethod( ACCESS_CONTROL_CONTEXT, persistenceClass, PERSISTENCE_UTIL_METHOD );
		...
		try {
			Object persistence = ReflectionHelper.newInstance( ACCESS_CONTROL_CONTEXT, persistenceClass, "persistence provider" );
		...
		try {
			...
			Class<? extends TraversableResolver> jpaAwareResolverClass = (Class<? extends TraversableResolver>)
					ReflectionHelper.loadClass( ACCESS_CONTROL_CONTEXT, JPA_AWARE_TRAVERSABLE_RESOLVER_CLASS_NAME, this.getClass() );
			jpaTraversableResolver = ReflectionHelper.newInstance( ACCESS_CONTROL_CONTEXT, jpaAwareResolverClass, "" );
			log.debugf(
